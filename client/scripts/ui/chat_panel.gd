...
func _build_ui() -> void:
	...
	_chat_input.anchors_preset = Control.PRESET_BOTTOM_LEFT
	_chat_input.offset_left = 10
	_chat_input.offset_bottom = -40
	_chat_input.offset_right = 360
	_chat_input.offset_top = -60
	_chat_input.placeholder_text = "按 Enter 或 / 聊天..."
...
func _input(event: InputEvent) -> void:
	# Enter 切换聊天输入
	if event.is_action_pressed("ui_accept") and not _chat_input.visible:
		if not _is_blocked_by_other_panels():
			_open_chat_input()
		get_viewport().set_chat_input_as_handled()
	
	# ESC 关闭输入
	if event.is_action_pressed("ui_cancel") and _chat_input.visible:
		_close_chat_input()
		get_viewport().set_chat_input_as_handled()
